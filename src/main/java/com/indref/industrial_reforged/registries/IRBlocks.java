...
public final class IRBlocks {
    ...
    public static final DeferredBlock<Block> REFRACTORY_BRICK = pickaxeMineable(registerBlockAndItem("refractory_brick",
            () -> new Block(BlockBehaviour.Properties.ofFullCopy(Blocks.SANDSTONE).mapColor(MapColor.GOLD))));
    ...
    public static final DeferredBlock<TerracottaBricks> TERRACOTTA_BRICKS = pickaxeMineable(registerBlockAndItem("terracotta_bricks",
            () -> new TerracottaBricks(BlockBehaviour.Properties.ofFullCopy(Blocks.TERRACOTTA))));
    ...
    private static <T extends Block> DeferredBlock<T> registerBlockAndItem(String name, Supplier<T> block, Function<DeferredBlock<T>, Supplier<BlockItem>> blockItem) {
        return registerBlockAndItem(name, block, blockItem, true, true, true);
    }
